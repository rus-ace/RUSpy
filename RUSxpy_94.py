...
class rus(QMainWindow, Ui_rus):
  ...
  def slope(self):
      ...
      print(self.delts, self.noise.value(), self.sweep_Hz)
...
##########################################################################################################
  ...
  def set_size(self):
    ...
    self.nSize.setValue(self.sweep_size)
    self.noise.setValue(0.2*Hz) #guess for slope parameter
...
############################################################################################################
  ...
  def write_cfg_settings(self, settings):
    ...
    settings.setValue('Terr', self.Terror)
  ...
  def read_dat(self): #read data from file
    ...
    if dialog.exec() == QDialog.Accepted: 
      ...
      self.reim.freq = freq
      self.reim.data = data
      Fstep = self.reim.freq[2] - self.reim.freq[1]
      Fstep = 1000*Fstep
      #print(Fstep)
      self.noise.setValue(0.2*Fstep) #guess for slope parameter
      fh.close 
