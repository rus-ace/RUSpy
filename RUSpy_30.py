...
class rus(QMainWindow, Ui_rus):
  ...
  def __init__(self):
    ...
    self.Tsw = True #controle writing data file
    self.old = True
  ...
  def TconTimer(self): # Timer for step control
    ...
    sTm = self.Stime.value()
    self.Tsw = False #blocks cursor from plotting
    self.old = False
  ...
  def set_Tenab(self,enabled):
    Twidg = [self.updtT, self.Tkc, self.bT, self.rP, self.Tstart, self.Tstep, self.nStep, self.Stime, \
      ...
      self.EndT, self.comID, self.Treset,  self.ClearT]
    for entry in Twidg:
      entry.setEnabled(enabled)
  ...
  def write_dat(self): #write data to file
    ...
    if self.auto == True:
      ...
      if comFlag == True:
        ...
        trds = treads.strip('\r\n')
        tflt = float(trds)
      fn = 'Data/'+trds + '_' +sx +'_' + dat + '_D.dat'
    else:
      dat = dt
      if comFlag == True:
