...
if comFlag == True:
    ...
    comport = p.device
    p = ports[0]
    ser = serial.Serial(comport, 9600, bytesize=8, timeout=1, parity=serial.PARITY_NONE, rtscts=1)  # open serial port 
    #time.sleep(5)
    ts = time.time()
    tout = []
    tout.append([])
...
####################################################################################
  ...
  def wFreq(self):
    ...
    with open(fn, 'w', newline='') as f:
        ...
        writer.writerows(b)
    axes3 = self.axes3
    axes3.plot(bx,by,color = 'black', marker = matplotlib.markers.CARETDOWNBASE , markersize =10, linestyle = 'None')
    self.canvas.draw()
  ...
  def plot_tout(self):
    ...
    ts1 = time.time()-ts
    tout[-1].append(ts1)
    self.axes3.plot(ts1,id,color = 'black', marker = 'o' , markersize =3, linestyle = 'None')
    self.canvas.draw()
...
class rus(QMainWindow, Ui_rus):
  ...
  def __init__(self):
    ...
    self.sSw.clicked.connect(self.Tsweep)
    ...
    self.setPt.setValue(int(id+0.5))
    if comFlag == True:
       self.set_Tenab(True)
    else:
  ...
  def set_enabled(self, enabled):
    widgets = [self.rateValue, self.level1Value, self.startValue, self.stopValue, self.stepValue, self.singleSweep, self.autoSweep, self.swpD]  
    for entry in widgets:
      entry.setEnabled(enabled)

  def set_Tenab(self,enabled):
    Twidg = [self.updtT, self.Tkc, self.setPt, self.bT, self.rP, self.Tstart, self.Tstep, self.nStep, self.Stime, self.EndT, self.sSw]
    for entry in Twidg:
      entry.setEnabled(enabled)
