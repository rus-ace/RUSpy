...
####################################################################################
  ...
  def on_xlims_change(self, axes3):
    ...
    km = self.zbox()[2]
    xhgt = self.zbox()[3]
    d = np.abs(self.rus.reim.data)#######################################3
    f = self.rus.reim.freq


    data1 = np.real(self.rus.reim.data[izl:izr])
    data2 = np.imag(self.rus.reim.data[izl:izr])
    ds1 = sum(data1)
    ds2 = sum(data2)
    dsq = math.sqrt(ds1*ds1+ds2*ds2)
    co = ds1/dsq
